...
async function start() {
  let game = await ask(
    "Let's play a game! Please choose either normal or reverse. (N/R)\n"
  );
  if (game === "N") {
    var minNum = await ask(
      "Enter the minimum number in the game's range (cannot be lower than 1):\n"
    );
    minNum = Number(minNum); // converts the user input from a string to a number. And sets the minimum.

    var maxNum = await ask(
      `Enter the maximum number in the game's range (must be greater than ${minNum}):\n`
    );
        ...
        if (answer2 === "H") {
          ...
          if (minNum >= maxNum) {
            let answer3 = await ask(
              `What do you mean Higher? I already guessed ${
                maxNum + 1
          ...
          if (minNum >= maxNum) {
            let answer3 = await ask(
              `What do you mean Lower? I already guessed ${
                minNum - 1
              }! Would you like to restart the game? (Y/N)?\n`
            );
            if (answer3 === "Y") {
    ...
    while (guess != secretNumber) {
      ...
      if (guess == secretNumber) {
        console.log(
          `Congratulations! You guessed the correct number. It took you ${attempts} attempts!`
        );
        let end = await ask(
          `Thanks for playing! Do you want to play again? (Y/N)?\n`
        );
